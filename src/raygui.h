...
typedef enum {
    ...
    TEXT_ALIGNMENT_VERTICAL,    // Text vertical alignment inside text bounds (after border and padding): 0-Top, 1-Middle, 2-Bottom
...
typedef enum {
    TEXT_READONLY = RAYGUI_MAX_PROPS_BASE,         // TextBox in read-only mode: 0-Text editable, 1-Text read-only
} GuiTextBoxProperty;
...
// TabBar
typedef enum {
    TAB_ITEMS_WIDTH = RAYGUI_MAX_PROPS_BASE,       // TabBar tab items width
...
// ColorPicker
typedef enum {
    COLOR_SELECTOR_SIZE = RAYGUI_MAX_PROPS_BASE,   // ColorPicker selector square size
